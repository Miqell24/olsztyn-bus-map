...
// Olsztyn: ONE feed from ZDZiT — 33 bus lines and the five tram lines of the
// network that reopened in 2015, the first new tram system built in Poland
// since the war. The feed ships NO SHAPES, so the stop sequence is the whole
// matching observation (the Grodzisk/Rybnik pseudo-matching path).
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|1,5]
...
// Exact-match repairs for poles a feed names badly. Olsztyn needs none — 845
// stop names, every one of them properly cased.
const NAME_FIX = {};
...
// ONE feed, TWO cfgs.
//
// ZDZiT Olsztyn publishes a dated GTFS on its own site (the Mobility Database
// mirror of it is a year stale, so download.sh resolves the newest file from
// the listing page instead). route_type 3 is the bus, 0 the tram.
//
// NO SHAPES at all — the file has no shapes.txt. The engine falls back to
// pseudo-matching, where the ordered stop sequence IS the observation the HMM
// rides, exactly as in Grodzisk and the Rybnik county feeds. It also means a
// misplaced pole does not merely put a dot in the wrong place: it drags the
// whole line into a detour, so STOP_FIX matters more here than usual.
//
...
// corridors, and the trams themselves run across the whole feed period —
// Budapest's pótló rule, and here the calendar proves it.
const LBL = new Map();
...
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/olsztyn-rail.json',
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      if ((feed.trolley && feed.trolley(r)) || r.route_type === '11') {
        ...
        // 0 is deliberately not in that list. ZDZiT paints its tram lines
        // #E31E1E, a red one shade off the family's #d6212b, and the family
        // rule is that a tram is the same red in every city.
