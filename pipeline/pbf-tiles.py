...
Olsztyn: 2 x 2 road tiles plus the tram network, which reopened in 2015 and
is mapped as railway=tram.
"""
import json, os, re, sys
import osmium

ROOT = os.path.join(os.path.dirname(__file__), '..')
PBFS = [os.path.join(ROOT, 'data', 'warminsko-mazurskie-latest.osm.pbf')]

# must match pipeline/download.sh
S, N, W, E = 53.68, 53.92, 20.25, 20.74
RAIL_BOX = (53.72, 20.4, 53.83, 20.56)
...
rail_file = os.path.join(ROOT, 'data/osm/olsztyn-rail.json')
...
GEN = 'pbf-tiles.py (Geofabrik warminsko-mazurskie)'
